...
public class VerificadorPlagioFrases implements VerificadorPlagio {
	
	String relatorio;
	
	public VerificadorPlagioFrases() {
		//this.relatorioFrases =  
	}
	
	public double comparaFrases(Tarefa submetida, Tarefa arquivada) {
		//FALTA UM METODO DE COMPARACAO AQUI
		//METODO PROVISORIO:
		Map<String, Double> relat = new HashMap<String, Double>();
		double soma = 0;
		
		List<Frase> paragrafosSub = submetida.frases;
		List<Frase> paragrafosArq = arquivada.frases;
	...
	public void comparaComTodasSubmissoes(Tarefa tarefa, String nomeTarefa) throws FileNotFoundException {
		...
		VerificadorPlagioFrases verificador = new VerificadorPlagioFrases();
		...
		for(Tarefa t : outrasSubmissoes) {
			double relatorio = verificador.comparaFrases(tarefa, t);
	...
	public double comparaIndividual(Tarefa tarefa1, Tarefa tarefa2) {
		...
		VerificadorPlagioFrases verificador = new VerificadorPlagioFrases();
		double relatorio = verificador.comparaFrases(tarefa1, tarefa2);
